...
def handle_coordinates():
    ...
    path_process = find_direction_and_adjust_angle('map.png')
    find_and_draw_path('map.png', (path_process[0][0],path_process[0][1]), (int(x),int(y)))
...
def find_and_draw_path(image_path, start, end):
    ...
    # 임계값 설정
    # 그냥 장애물 무시한다 싶으면 이임계값을바꿔라 기본값은 128
    # 그냥 장애물 무시한다 싶으면 이임계값을바꿔라 중요하니까 2번말했다
    threshold = 128
    # 그냥 장애물 무시한다 싶으면 이임계값을바꿔라 중요하니까 3번말했다
    # 그냥 장애물 무시한다 싶으면 이임계값을바꿔라 중요하니까 4번 말했다
    ...
    for i in range(len(simplified_coords) - 1):
        vector = np.array(simplified_coords[i+1]) - np.array(simplified_coords[i])
        direction = np.arctan2(vector[1], vector[0])  # 방향 (라디안)
        distance = np.linalg.norm(vector)  # 거리
        path_process.append((distance, np.degrees(direction)))  # 거리와 방향을 도 단위로 변환하여 추가

    # 함수 외부에서도 그래프 G를 사용할 수 있도록 반환
    return G, path
